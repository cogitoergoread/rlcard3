...
class Env(object):
    ...
    def __init__(self, config):
        ...
        print(f"EnvInit, StepBack:{self.allow_step_back}")
        ...
        self.multi_agent_mode = config['multi_agent_mode']

        # Load pre-trained models if either single_agent_mode=True
        if self.single_agent_mode or self.human_mode or self.multi_agent_mode:
    ...
    def step_back(self):
        ...
        if not self.allow_step_back:
            raise Exception('Step back is off. To use step_back, please set allow_step_back=True in rlcard3.make')
    ...
    def reset(self):
        ...
        if self.human_mode:
            print('\n>> Start a new game!')
            print(f"You are player {self.active_player}")
        while True:
            state, player_id = self.game.init_game()
            while not player_id == self.active_player:
                ...
                if self.human_mode:
                    agent_name = self.model.agents[player_id].__str__()
                    print('\r>> Agent {} is {}, chooses '.format(player_id, agent_name), end='')
                    self.print_action(action)
                # Original code from rlcard3
    ...
    def _single_agent_step(self, action):
        ...
        while not self.game.is_over() and not player_id == self.active_player:
            ...
            if self.human_mode:
                agent_name = self.model.agents[player_id].__str__()
                print('\r>> Agent {} is {}, chooses '.format(player_id, agent_name), end='')
                self.print_action(action)  # It is only for mocsár::::
                print('')
            # Original code from RLCard3
            #            action, _ = self.model.agents[player_id].eval_step(self._extract_state(state))
            #            if not self.model.agents[player_id].use_raw:
